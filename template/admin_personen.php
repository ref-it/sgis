...
<form action="<?php echo $_SERVER["PHP_SELF"];?>#person" method="POST" class="tr" style="background-color: lightyellow;">
 ...
 <div class="td"><select name="filter_personen_unirzlogin[]" multiple="multiple"><?php foreach ($filter["unirzlogin"] as $unirzlogin): ?><option <?if (in_array($unirzlogin, $activefilter["unirzlogin"])):?> selected="selected"<? endif;?>><?=$unirzlogin;?></option><?php endforeach;?></select></div>
 <div class="td"><select name="filter_personen_username[]" multiple="multiple"><?php foreach ($filter["username"] as $username): ?><option <?if (in_array($username, $activefilter["username"])):?> selected="selected"<? endif;?>><?=$username;?></option><?php endforeach;?></select></div>
 <div class="td"><select name="filter_personen_lastLogin[]" multiple="multiple"><?php foreach ($filter["lastLogin"] as $lastLogin): ?><option <?if (in_array($lastLogin, $activefilter["lastLogin"])):?> selected="selected"<? endif;?>><?=$lastLogin;?></option><?php endforeach;?></select></div>
<div class="td"><select name="filter_personen_canLogin[]" multiple="multiple"><?php foreach ($filter["canLogin"] as $v => $canLogin): ?><option <?if (in_array($v, $activefilter["canLogin"])):?> selected="selected"<? endif;?> value="<?=htmlspecialchars($v);?>"><?=$canLogin;?></option><?php endforeach;?></select></div>
 <div class="td"><select name="filter_personen_active[]" multiple="multiple"><?php foreach ($filter["active"] as $v => $active): ?><option <?if (in_array($v, $activefilter["active"])):?> selected="selected"<? endif;?> value="<?=htmlspecialchars($v);?>"><?=$active;?></option><?php endforeach;?></select></div>
</form>
...
?>
<div class="tr" id="rowP<?=$person["id"];?>">
 <div class="td">
   <?=$i;?>.
...
foreach($gremien as $gremium):
?>
     <div class="tr <?=($gremium["active"] ? "gremiumactive" : "gremiuminactive");?> forperson<?=$person["id"];?>">
      <div class="td">
...
?>
     ...
     <a href="#" onClick="$('div.tr.gremiuminactive.forperson<?=$person["id"];?>').toggle(); return false;">[inaktive Zuordnungen anzeigen/ausblenden]</a>
   </div>
   <?php
...
?><li><a href="<?=htmlentities($_SERVER["PHP_SELF"]);?>?person_start=<?=$i;?>#person" title="<?=htmlspecialchars($struct_personen[$i]["email"]);?>"><?=$i;?></a></li><?
  endif;
  if ($i < $prev && $i + $_COOKIE["person_length"] > $prev): 
?><li><a href="<?=htmlentities($_SERVER["PHP_SELF"]);?>?person_start=<?=$prev;?>#person" title="<?=htmlspecialchars($struct_personen[$prev]["email"]);?>">&lt;</a></li><?
  endif;
...
?><li><a href="<?=htmlentities($_SERVER["PHP_SELF"]);?>?person_start=<?=$next;?>#person" title="<?=htmlspecialchars($struct_personen[$next]["email"]);?>">&gt;</a></li><?
  endif;
 endfor;
endif; ?>
