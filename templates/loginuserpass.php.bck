...
if ($this->data['rememberPasswordEnabled']) {
	echo str_repeat("\t", 4);
	echo '<input type="checkbox" id="remember_password" tabindex="4" name="remember_password" value="Yes" ';
	echo ($this->data['rememberPasswordChecked'] ? 'checked="Yes" /> ' : '/> ');
	echo $this->t('{sgis:sgis:remember_password}');
} elseif ($this->data['rememberUsernameEnabled']) {
	$text = $this->t('{login:login_button}');
	echo str_repeat("\t", 4);
	echo "<input type=\"submit\" tabindex=\"4\" value=\"{$text}\" />";
}
...
<?php
	...
	if ($this->data['rememberPasswordEnabled'] && $this->data['rememberUsernameEnabled']) {
		$text = $this->t('{login:login_button}');
		echo str_repeat("\t", 4);
		echo "<tr><td colspan=\"3\"><input type=\"submit\" tabindex=\"4\" value=\"{$text}\" /></td></tr>";
	}
}
?>
