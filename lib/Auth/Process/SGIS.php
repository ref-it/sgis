...
namespace SimpleSAML\Module\sgis\Auth\Process;
use \PDO;
 ...
 * @version $Id$
 */
class SGIS extends \SimpleSAML\Auth\ProcessingFilter {
    ...
    * The config object.
    ...
    public function __construct($config, $reserved) {
        ...
        if (!isset($config['dsn'])) {
            throw new SimpleSAML\Error\Exception($this->authId . ': Missing required \'dsn\' option.');
        }
        if (!isset($config['username'])) {
            throw new SimpleSAML\Error\Exception($this->authId . ': Missing required \'username\' option.');
        }
        if (!isset($config['password'])) {
            throw new SimpleSAML\Error\Exception($this->authId . ': Missing required \'password\' option.');
        }
        if (!isset($config['prefix'])) {
            throw new SimpleSAML\Error\Exception($this->authId . ': Missing required \'prefix\' option.');
        }
        if (!isset($config['unimail'])) {
            throw new SimpleSAML\Error\Exception($this->authId . ': Missing required \'unimail\' option.');
        }
        if (!is_array($config['unimail'])) {
            $config['unimail'] = [ $config['unimail'] ];
        }
        if (!isset($config['unildaphost'])) {
            throw new SimpleSAML\Error\Exception($this->authId . ': Missing required \'unildaphost\' option.');
        }
        if (!isset($config['unildapbase'])) {
            throw new SimpleSAML\Error\Exception($this->authId . ': Missing required \'unildapbase\' option.');
        }
