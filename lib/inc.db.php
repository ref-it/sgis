...
if ($r === false) {
  $pdo->query("CREATE TABLE ${DB_PREFIX}rel_rolle_gruppe (
                ...
                PRIMARY KEY (rolle_id, gruppe_id) ) ENGINE=INNODB CHARACTER SET utf8 COLLATE utf8_general_ci;") or die(print_r($pdo->errorInfo(),true));
...
if ($r === false) {
  $pdo->query("CREATE TABLE ${DB_PREFIX}rel_rolle_mailingliste (
                ...
                PRIMARY KEY (rolle_id, mailingliste_id) ) ENGINE=INNODB CHARACTER SET utf8 COLLATE utf8_general_ci;") or die(print_r($pdo->errorInfo(),true));
...
if ($r === false) {
  $pdo->query("CREATE TABLE ${DB_PREFIX}rel_mitgliedschaft (
		...
		gremium_id INT NOT NULL,
                ...
                PRIMARY KEY (id) ) ENGINE=INNODB CHARACTER SET utf8 COLLATE utf8_general_ci;") or die(print_r($pdo->errorInfo(),true));
...
function getPersonDetailsByMail($mail) {
  ...
  $query->execute(Array($mail)) or die(print_r($query->errorInfo(),true));
...
function getPersonRolle($personId) {
  ...
  $query->execute(Array($personId)) or die(print_r($query->errorInfo(),true));
...
function setPersonUsername($personId, $username) {
  ...
  return $query->execute(Array($username, $personId)) or die(print_r($query->errorInfo(),true));
}
...
function setPersonPassword($personId, $password) {
  ...
  return $query->execute(Array($passwordHash, $personId)) or die(print_r($query->errorInfo(),true));
}
...
function dbMailinglisteInsert($address, $url, $password) {
  ...
  return $query->execute(Array($address, $url, $password)) or die(print_r($query->errorInfo(),true));
}
...
function dbMailinglisteUpdate($id, $address, $url, $password) {
  ...
  return $query->execute(Array($address, $url, $password, $id)) or die(print_r($query->errorInfo(),true));
}
...
function getMailinglisteRolle($mlId) {
  ...
  $query->execute(Array($mlId)) or die(print_r($query->errorInfo(),true));
  return $query->fetchAll(PDO::FETCH_ASSOC);
}
...
function dbMailinglisteDropRolle($mlId, $rolleId) {
  ...
  return $query->execute(Array($mlId, $rolleId)) or die(print_r($query->errorInfo(),true));
}
...
function getAlleRolle() {
  global $pdo, $DB_PREFIX;
  $query = $pdo->prepare("SELECT DISTINCT g.id AS gremium_id, g.name as gremium_name, g.fakultaet as gremium_fakultaet, g.studiengang as gremium_studiengang, g.studiengangabschluss as gremium_studiengangabschluss, g.wiki_members as gremium_wiki_members, r.id as rolle_id, r.name as rolle_name FROM ${DB_PREFIX}gremium g LEFT JOIN ${DB_PREFIX}rolle r ON g.id = r.gremium_id ORDER BY g.name, g.fakultaet, g.studiengang, g.studiengangabschluss, g.id, r.name, r.id");
...
function getAllePerson() {
  global $pdo, $DB_PREFIX;
  $query = $pdo->prepare("SELECT * FROM ${DB_PREFIX}person p ORDER BY RIGHT(email, LENGTH(email) - POSITION( '@' in email)), LEFT(email, POSITION( '@' in email))");
  $query->execute(Array()) or die(print_r($query->errorInfo(),true));
  return $query->fetchAll(PDO::FETCH_ASSOC);
}
...
function dbPersonUpdate($id,$name,$email,$unirzlogin,$username,$password,$canlogin) {
  ...
  $ret1 = $query->execute(Array($name, $email, $unirzlogin, $username, $canlogin, $id)) or die(print_r($query->errorInfo(),true));
  if (empty($password)) {
    $ret2 = true;
  } else {
    $passwordHash = @$pwObj->createPasswordHash($password);
    $query = $pdo->prepare("UPDATE ${DB_PREFIX}person SET password = ? WHERE id = ?");
    $ret2 = $query->execute(Array($passwordHash, $id)) or die(print_r($query->errorInfo(),true));
  }
  $ret3 = $pdo->commit() or die(print_r($pdo->errorInfo(),true));
  return $ret1 && $ret2 && $ret3;
}
...
function dbPersonInsert($name,$email,$unirzlogin,$username,$password,$canlogin) {
  ...
  return $query->execute(Array($name, $email, $unirzlogin, $username, $passwordHash, $canlogin)) or die(print_r($query->errorInfo(),true));
}
...
function dbPersonInsertRolle($person_id,$rolle_id,$von,$bis,$beschlussAm,$beschlussDurch,$kommentar) {
  ...
  $query->execute(Array($rolle_id)) or die (print_r($query->errorInfo(),true));
  ...
  return $query->execute(Array($person_id,$rolle_id,$gremium_id,$von,$bis,$beschlussAm,$beschlussDurch,$kommentar)) or die(print_r($query->errorInfo(),true));
}
...
function dbPersonUpdateRolle($id, $person_id,$rolle_id,$von,$bis,$beschlussAm,$beschlussDurch,$kommentar) {
  ...
  return $query->execute(Array($person_id,$rolle_id,$gremium_id,$von,$bis,$beschlussAm,$beschlussDurch,$kommentar,$id)) or die(print_r($query->errorInfo(),true));
}
...
function getGruppeRolle($grpId) {
  ...
  $query->execute(Array($grpId)) or die(print_r($query->errorInfo(),true));
  return $query->fetchAll(PDO::FETCH_ASSOC);
}
...
function dbGruppeInsert($name, $beschreibung) {
  ...
  return $query->execute(Array($name, $beschreibung)) or die(print_r($query->errorInfo(),true));
}

function dbGruppeUpdate($id, $name, $beschreibung) {
  global $pdo, $DB_PREFIX;
  $query = $pdo->prepare("UPDATE ${DB_PREFIX}gruppe SET name = ?, beschreibung = ? WHERE id = ?");
  return $query->execute(Array($name, $beschreibung, $id)) or die(print_r($query->errorInfo(),true));
}
...
function dbGruppeDropRolle($grpId, $rolleId) {
  ...
  return $query->execute(Array($grpId, $rolleId)) or die(print_r($query->errorInfo(),true));
}
...
function dbGremiumInsert($name, $fakultaet, $studiengang, $studiengangabschluss, $wiki_members) {
  global $pdo, $DB_PREFIX;
echo __LINE__."<br>\n";
  var_dump(Array($name, $fakultaet, $studiengang, $studiengangabschluss, $wiki_members));
  $query = $pdo->prepare("INSERT ${DB_PREFIX}gremium (name, fakultaet, studiengang, studiengangabschluss, wiki_members) VALUES ( ?, ?, ?, ?, ?)");
  return $query->execute(Array($name, $fakultaet, $studiengang, $studiengangabschluss, $wiki_members)) or die(__FILE__.":".__LINE__." ".print_r($query->errorInfo(),true));
}

function dbGremiumUpdate($id, $name, $fakultaet, $studiengang, $studiengangabschluss, $wiki_members) {
  global $pdo, $DB_PREFIX;
  $query = $pdo->prepare("UPDATE ${DB_PREFIX}gremium SET name = ?, fakultaet = ?, studiengang = ?, studiengangabschluss = ?, wiki_members = ? WHERE id = ?");
  return $query->execute(Array($name, $fakultaet, $studiengang, $studiengangabschluss, $wiki_members, $id)) or die(print_r($query->errorInfo(),true));
}
...
function dbGremiumInsertRolle($gremium_id, $name) {
  global $pdo, $DB_PREFIX;
  $query = $pdo->prepare("INSERT INTO ${DB_PREFIX}rolle (gremium_id, name) VALUES ( ?, ?)");
  return $query->execute(Array($gremium_id, $name)) or die(print_r($query->errorInfo(),true));
}

function dbGremiumUpdateRolle($id, $name) {
  global $pdo, $DB_PREFIX;
  $query = $pdo->prepare("UPDATE ${DB_PREFIX}rolle SET name = ? WHERE id = ?");
  return $query->execute(Array($name, $id)) or die(print_r($query->errorInfo(),true));
}
...
function getRolleMailinglisten($rolleId) {
  ...
  $query->execute(Array($rolleId)) or die(print_r($query->errorInfo(),true));
  return $query->fetchAll(PDO::FETCH_ASSOC);
}
...
function getActiveMitgliedschaftByMail($email, $rolleId) {
  ...
  $query->execute(Array($email, $rolleId)) or die(print_r($query->errorInfo(),true));
  return $query->fetchAll(PDO::FETCH_ASSOC);
}

function getAllMitgliedschaft() {
  global $pdo, $DB_PREFIX;
  $query = $pdo->prepare("SELECT g.name as gremium_name, g.fakultaet as gremium_fakultaet, g.studiengang as gremium_studiengang, g.studiengangabschluss as gremium_studiengangabschluss, r.name as rolle_name, p.email as person_email, p.name as person_name, p.username as person_username, rm.von as von, rm.bis as bis, rm.beschlussAm as beschlussAm, rm.beschlussDurch as beschlussDurch, rm.kommentar as kommentar FROM {$DB_PREFIX}person p INNER JOIN {$DB_PREFIX}rel_mitgliedschaft rm ON rm.person_id = p.id INNER JOIN {$DB_PREFIX}gremium g ON g.id = rm.gremium_id INNER JOIN {$DB_PREFIX}rolle r ON r.id = rm.rolle_id ORDER BY g.name, g.id, r.name, r.id, RIGHT(p.email, LENGTH(p.email) - POSITION( '@' in p.email)), LEFT(p.email, POSITION( '@' in p.email))");
  $query->execute() or die(print_r($query->errorInfo(),true));
  return $query->fetchAll(PDO::FETCH_ASSOC);
}
