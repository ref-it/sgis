...
require_once (dirname(__FILE__) . '/class.Crypto.php');
require_once (dirname(__FILE__) . '/class._JsonController.php');
require_once (dirname(__FILE__) . '/class._MotherController.php');
...
class FileHandler extends MotherController {
	...
	/**
	 * path to DATABASE filecache or FILESYSTEM storage - no '/' at the ends
	...
	/**
	 * disable db components on file upload
	 * default: false - may overwritten by global constants
	 * be careful with this option
	 * @var bool
	 */
	 private $UPLOAD_IGNORE_DB;

	/**
	 * only used if db is ignored on upload
	 * ignore file exist check
	 * default: false - may overwritten by global constants
	 * be careful with this option
	 * @var bool
	 */
	private $UPLOAD_FORCE_OVERWRITE;
	...
	function initSettings($settings){
		...
		(isset($settings['UPLOAD_MULTIFILE_BREAOK_ON_ERROR'])?
			...
			:(defined('UPLOAD_MULTIFILE_BREAOK_ON_ERROR')?
				UPLOAD_MULTIFILE_BREAOK_ON_ERROR
				: true));
		$this->UPLOAD_DISK_PATH =
		(isset($settings['UPLOAD_DISK_PATH'])?
			$settings['UPLOAD_DISK_PATH']
			:(defined('UPLOAD_DISK_PATH')?
				UPLOAD_DISK_PATH
				: ''));
		$this->UPLOAD_IGNORE_DB =
 		(isset($settings['UPLOAD_IGNORE_DB'])?
			$settings['UPLOAD_IGNORE_DB']
			:(defined('UPLOAD_IGNORE_DB')?
				UPLOAD_IGNORE_DB
				: false));
		$this->UPLOAD_FORCE_OVERWRITE =
		(isset($settings['UPLOAD_FORCE_OVERWRITE'])?
			$settings['UPLOAD_FORCE_OVERWRITE']
			:(defined('UPLOAD_FORCE_OVERWRITE')?
				UPLOAD_FORCE_OVERWRITE
				: false));
	...
	function __construct($db, $settings = NULL){
		...
		if (!class_exists('finfo') && !class_exists('\finfo')){
			ErrorHandler::_errorExit('Server function finfo does not exist. Please install the plugin');
		}
	}
	...
	public function deleteFileByHash($hash){
		...
		if ($file){
			...
			$path = $this->_getDiskpathOfFile($file);
	...
	public function deliverFileData($file, $noinline = false){
		if (!$this->UPLOAD_TARGET_DATABASE){ // disk FILESYSTEM storage ------------
			if (!file_exists($this->_getDiskpathOfFile($file))){
				error_log("FILE Error: File not found on disk. File Id: {$file->id} File Path: ". $this->_getDiskpathOfFile($file) );
			} else {
				...
				if (self::hasModXSendfile()){
					header("X-Sendfile: ".$this->_getDiskpathOfFile($file));
					return;
				} else {
					header('Content-Disposition: '.(!$noinline?'inline':'attachment').'; filename="'.$file->filename.(($file->fileextension)?'.'.$file->fileextension:'').'"');
					echo file_get_contents($this->_getDiskpathOfFile($file));
					return;
				}
			}
		} else { // DATABASE storage ----------------
			if ($this->UPLOAD_USE_DISK_CACHE && file_exists($this->_getDiskpathOfFile($file))){
				header('Last-Modified: '.$file->getAddedOnDate()->format('D, d M Y H:i:s').' GMT', true);
				if ($file->size) header('Content-Length: ' . $file->size );
				if ($file->mime){
					header("Content-Type: {$file->mime}");
				...
				if ($this->UPLOAD_USE_DISK_CACHE){
					self::checkCreateDirectory($this->_getDirpathOfFile($file));
					file_put_contents($this->_getDiskpathOfFile($file), $data);
					// apache deliver
					if (self::hasModXSendfile()){
						$this->close_db_file();
						// apache deliver
	...
	public function getFiledataBinary($file, $cache = true){
		if (!$this->UPLOAD_TARGET_DATABASE){ // disk FILESYSTEM storage ------------
			if (!file_exists($this->_getDiskpathOfFile($file))){
				...
				return file_get_contents($this->_getDiskpathOfFile($file));
			}
		} else { // DATABASE storage ----------------
			if ($this->UPLOAD_USE_DISK_CACHE && file_exists($this->_getDiskpathOfFile($file))){
				return file_get_contents($this->_getDiskpathOfFile($file));
	 ...
	 * @return string
	 */
	public function _getBaseDirPath(){
		if (!$this->UPLOAD_DISK_PATH) {
			return self::getBaseDirPath();
		} else {
			return $this->UPLOAD_DISK_PATH;
		}
	}
	 ...
	 * @return string
	 */
	public function _getDirpathOfFile($file){
	 ...
	 * @return string
	 */
	public function _getDiskpathOfFile($file){
	 	return $this::_getDirpathOfFile($file).'/'. $file->hashname;
	}
	...
	public function upload($link, $base_key = 'file'){
		...
		if (!isset($_POST['nofile']) && isset($_FILES) && count($_FILES) > 0 &&
			...
			if (count($_FILES[$base_key]['name']) > $this->UPLOAD_MAX_MULTIPLE_FILES){
				...
				foreach ( $_FILES[$base_key]['name'] as $id => $filename ){
					...
					// hashname
					$file->hashname = strtolower(Crypto::generateRandomString(32));
					...
					if ($file->mime && mb_substr($file->mime, 0, 5) == 'text/'){
						...
						$enc = $finfo2->file($_FILES[$base_key]['tmp_name'][$id]);
						if ($enc){
							$file->encoding = $enc;
						}
					}
			...
			if ($this->UPLOAD_MULTIFILE_BREAOK_ON_ERROR && count($result['error']) > 0){
				...
				foreach ($files as $tmp_name => $file){
					...
					// check db for existing files
					if (!$this->UPLOAD_FORCE_OVERWRITE){
						//scandir
						$dir = $this->_getBaseDirPath();
						$files = array_slice(scandir($dir), 2);

						foreach ($files as $tmp_name => $file) {
							if (in_array($file->filename.'.'.$file->fileextension, $files, true)) {
								$result['error'][] = "The File '$file->filename' already exists on this path.";
								unset($files[$tmp_name]);
							}
						}
		...
		if (count($result['fileinfo']) == 0){
			...
			if (!$this->UPLOAD_TARGET_DATABASE){
				foreach ( $result['fileinfo'] as $tmp_name => $file ){
					$dir = $this->_getDirpathOfFile($file);
					// create directory if not extists
					self::checkCreateDirectory($dir);
					// upload file
					$uploadfile = $this->_getDiskpathOfFile($file);
					// move file to directory
					if (move_uploaded_file($tmp_name, $uploadfile)) {
						$dberror = NULL;
						//create file entry
						$file->id = (!$this->UPLOAD_IGNORE_DB)? $this->db->createFile($file) : 1;
						$dberror = (!$this->UPLOAD_IGNORE_DB)? $this->db->isError() : false;
						//create data entry
						if (!$dberror && !$this->UPLOAD_IGNORE_DB){
						...
						if ($dberror) {
							...
							ErrorHandler::_errorLog('DB Error on File upload. Error: '.$this->db->getError().' --- Values: "'.json_encode((array) $file));
							$result['error'][] = "DB Error -> remove from disk";
	 ...
	 * @param string $b string binary string representation
	 * @return string
	 */
